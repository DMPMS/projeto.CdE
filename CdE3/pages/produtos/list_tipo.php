...
if (isset($_SESSION['id']) == '') {
    ...
    $sql = "SELECT * FROM produto_tipos WHERE ativo = 0";
    ...
    if (isset($data['excluir']) == 'excluir') {
        ...
        //Excluir
        $sqlExcluir = "UPDATE produto_tipos SET ativo = ? WHERE id = ?";
        $qExcluir = $pdo->prepare($sqlExcluir);
        $qExcluir->execute(array(1, $data['id']));

        //Atualização
        $sqlAtualizacao = "INSERT INTO atualizacao_atualizacoes (tipo, acao, id_tipo, id_responsavel, ids_vizualizados, criadoem) values(?,?,?,?,?,?)";
        $qAtualizacao = $pdo->prepare($sqlAtualizacao);
        $qAtualizacao->execute(array("Produto", "Excluir-Tipo", $data['id'], $_SESSION['id'], ".{$_SESSION['id']}.", date('Y-m-d H:i:s')));

        Database::disconnect();

        $_SESSION['Excluir'] = NomeTipo($data['id']);
        echo '<script>
            ...
            <?php if (isset($_SESSION['Excluir'])) { ?>
                ...
                <script>
                    window.onload = function () {
                        $.toast({
                            text: '<b><?php echo $_SESSION['Excluir']; ?></b> excluído.',
                            icon: 'success',
                            hideAfter: 5000,
                            loader: false,
                            position: 'top-right'
                        ...
                        <div class="container-fluid">
                            <div class="page-header">
                                <div class="row">
                                    <div class="col-lg-12">
                                        ...
                                        <div class="page-header-title">
                                            <i class="ik ik-server bg-green"></i>
                                        </div>
                                            ...
                                            <tbody>
                                                <?php foreach ($pdo->query($sql) as $row) { ?>
                                                    <tr>
                                                        ...
                                                        <td><?php echo QtdProdutosDoTipo($row['id']); ?></td>
                                                        <td class="text-center">
                                                            ...
                                                            <a title="Produtos" href="list_tipo_produto.php?id=<?php echo $row['id']; ?>" class="btn btn-icon btn-success mr-1"><i class="ik ik-package"></i></a>
                                                            <?php if ($_SESSION['tipo'] == "Administrador Geral") { ?>
                                                                <a title="Editar" href="edit_tipo.php?id=<?php echo $row['id']; ?>" class="btn btn-icon btn-warning mr-1"><i class="ik ik-edit-2"></i></a>
                                                                <?php if (QtdProdutosDoTipo($row['id']) > 0) { ?>
                                                                    <button title="Informação" type="button" class="btn btn-icon btn-info" data-toggle="modal" data-target="#Informacao<?php echo $row['id']; ?>"><i class="ik ik-info"></i></button>
                                                                <?php } else { ?>
                                            ...
                                            <div class="modal-body">
                                                <div class="row">
                                                    ...
                                                    <div class="form-group col-lg-2">
                                                        <label>Produtos <a href="list_tipo_produto.php?id=<?php echo $row['id']; ?>"><i class="ik ik-arrow-right-circle text-primary"></i></a></label>
                                                        <input type="text" class="form-control" readonly="" value="<?php echo QtdProdutosDoTipo($row['id']); ?>">
                                                    </div>
                                                    <div class="form-group col-lg-3">
                                                        <label>Nome</label>
                                                        <input type="text" class="form-control" readonly="" value="<?php echo $row['nome']; ?>">
                                                    </div>
                                                </div>
                                            </div>
                            ...
                            <?php if ($_SESSION['tipo'] == "Administrador Geral") { ?>
                                ...
                                foreach ($pdo->query($sql) as $row) {
                                    if (QtdProdutosDoTipo($row['id']) > 0) {
                                        ?>
                                        <div class="modal fade" id="Informacao<?php echo $row['id']; ?>">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    ...
                                                    <form method="POST" enctype="multipart/form-data">
                                                        <div class="modal-body">
                                                            <p>Para excluir <b><?php echo $row['nome']; ?></b>, todos os produtos vinculados a ele devem ser desvinculados. Clique <a href="list_tipo_produto.php?id=<?php echo $row['id']; ?>" class="text-primary">aqui</a> para ver os produtos.</p>
                                                        </div>
                                        ...
                                        <div class="modal fade" id="Excluir<?php echo $row['id']; ?>">
                                            <div class="modal-dialog modal-dialog-centered">
                                                <div class="modal-content">
                                                    ...
                                                    <form method="POST" enctype="multipart/form-data">
                                                        <div class="modal-body">
                                                            <p>Deseja excluir <b><?php echo $row['nome']; ?></b>? Não há <strong>nenhum</strong> produto vinculado a este tipo.</p>
                                                        </div>
                                                        <input type="hidden" name="id" value="<?php echo $row['id']; ?>">
                                                        <div class="modal-footer">
