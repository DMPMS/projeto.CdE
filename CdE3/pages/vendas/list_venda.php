...
if (isset($_SESSION['id']) == '') {
    ...
    $sql = "SELECT * FROM venda_vendas WHERE ativo = 0";

    $data = filter_input_array(INPUT_POST, FILTER_DEFAULT);
    if (isset($data['cancelar']) == 'cancelar') {
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

        //Cancelar
        $sqlCancelar = "UPDATE venda_vendas SET ativo = ?, id_responsavel_cancelamento = ?, data_cancelamento = ? WHERE id = ?";
        $qCancelar = $pdo->prepare($sqlCancelar);
        $qCancelar->execute(array(1, $_SESSION['id'], date('Y-m-d H:i:s'), $data['id']));

        //Atualização
        $sqlAtualizacao = "INSERT INTO atualizacao_atualizacoes (tipo, acao, id_venda, id_responsavel, ids_vizualizados, criadoem) values(?,?,?,?,?,?)";
        $qAtualizacao = $pdo->prepare($sqlAtualizacao);
        $qAtualizacao->execute(array("Venda", "Cancelar-Venda", $data['id'], $_SESSION['id'], ".{$_SESSION['id']}.", date('Y-m-d H:i:s')));

        Database::disconnect();

        $_SESSION['Cancelar'] = $data['id'];
        echo '<script>
                if ( window.history.replaceState ) {
                    window.history.replaceState( null, null, window.location = "list_venda_canceladas.php");
                }
            </script>';
    }
?>
    ...
    <head>
        ...
        <?php if (isset($_SESSION['Venda-Cadastrada'])) { ?>
            <!--Notificação Venda Realizada-->
            <script>
                window.onload = function() {
                    $.toast({
                        text: '<b>Venda <?php echo UltimoIdVenda(); ?></b> realizada com sucesso.',
                        icon: 'success',
                        hideAfter: 5000,
                        loader: false,
                        position: 'top-right'
                ...
                <div class="main-content">
                    <div class="container-fluid">
                        <div class="page-header">
                            <div class="row">
                                <div class="col-lg-4">
                                    ...
                                    <div class="page-header-title">
                                        <i class="ik ik-list bg-orange"></i>
                                    </div>
                                </div>
                                <div class="col-lg-8">
                                            ...
                                            <?php foreach ($pdo->query($sql) as $row) { ?>
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        ...
                                                        <button title="Cancelar" type="button" class="btn btn-icon btn-danger" data-toggle="modal" data-target="#Cancelar<?php echo $row['id']; ?>"><i class="ik ik-x-circle"></i></button>
                                                    </td>
                                                </tr>
                                            <?php } ?>
                                        </tbody>
                        ...
                        <?php if ($_SESSION['tipo'] == "Administrador Geral") { ?>
                            <!--Modal de Cancelar-->
                            <?php foreach ($pdo->query($sql) as $row) { ?>
                                <div class="modal fade" id="Cancelar<?php echo $row['id']; ?>">
                                    <div class="modal-dialog modal-dialog-centered">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h5 class="modal-title">Cancelar Venda <strong><?php echo $row['id']; ?></strong></h5>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                            <form method="POST" enctype="multipart/form-data">
                                                <div class="modal-body">
                                                    <p>Deseja cancelar a <b>Venda <?php echo $row['id']; ?></b>? Após cancelar esta venda, ela nunca mais poderá ser considerada para os cálculos das receitas (Diária, Semanal, Mensal, Anual e Total). Ao cancelar a venda, o estoque não será reabastecido automaticamente. Caso deseje que seja reabastecido, essa tarefa deverá ser realizada manualmente pelo administrador.</p>
                                                </div>
                                                <input type="hidden" name="id" value="<?php echo $row['id']; ?>">
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Não</button>
                                                    <button type="submit" name="cancelar" value="cancelar" class="btn btn-danger">Sim</button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
        ...
        <?php if (isset($_SESSION['Venda-Cadastrada'])) { ?>
            ...
            <script type="text/javascript">
                $(window).on('load', function() {
                    $('#Dados<?php echo UltimoIdVenda(); ?>').modal('show');
                });
            </script>
        <?php
            unset($_SESSION['Venda-Cadastrada']);
        } ?>
    </body>
